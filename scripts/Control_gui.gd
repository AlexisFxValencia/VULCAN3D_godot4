...
func _ready():
	...
	MFR_x_slider.connect("value_changed", Callable(self, "_set_MFR_x"))
	...
	MFR_y_slider.connect("value_changed", Callable(self, "_set_MFR_y"))
	...
	MFR_z_slider.connect("value_changed", Callable(self, "_set_MFR_z"))
	...
	speed_slider.connect("value_changed", Callable(self, "change_gui_speed"))
	container.add_child(speed_slider)
	
	b2m_label = Label.new()
	b2m_label.set_text("B2m = " + str(get_parent().get_node("Area3D").b2m))
	...
	if DRAGGABLE:
		...
		for child in container.get_children():
			child.connect("mouse_entered", Callable(self, "_panel_entered"))
			child.connect("mouse_exited", Callable(self, "_panel_exited"))
...
func _compute_b2g():
	var vector = get_parent().get_node("Area3D/CollisionShape3D").shape.extents
	b2g = pow(PI/vector.x,2) + pow(PI/vector.y,2) + pow(PI/vector.z,2)

	
func _set_MFR_x(val):
	get_parent().get_node("Area3D/CollisionShape3D").shape.extents.x = 2*val
	get_parent().get_node("Area3D/MFR").mesh.size.x = 4*val
	_compute_b2g()
	b2g_label.set_text("B2g = " + str(b2g))
	
func _set_MFR_y(val):
	get_parent().get_node("Area3D/CollisionShape3D").shape.extents.y = 2*val
	get_parent().get_node("Area3D/MFR").mesh.size.y = 4*val
	_compute_b2g()
	b2g_label.set_text("B2g = " + str(b2g))
	
func _set_MFR_z(val):
	get_parent().get_node("Area3D/CollisionShape3D").shape.extents.z = 2*val
	get_parent().get_node("Area3D/MFR").mesh.size.z = 4*val
	_compute_b2g()
	b2g_label.set_text("B2g = " + str(b2g))
	
func change_gui_speed(val):
	print("changing speed")
	get_parent().get_node("Camera3D").max_speed.x = val
	get_parent().get_node("Camera3D").max_speed.y = val
	get_parent().get_node("Camera3D").max_speed.z = val
