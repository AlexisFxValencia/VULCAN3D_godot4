...
@export var mouse_mode = Input.MOUSE_MODE_VISIBLE#= Input.MOUSE_MODE_CAPTURED # (int, "Visible", "Hidden", "Captured, Confined")
...
func _update_views_physics(delta):
	...
	var query = PhysicsRayQueryParameters3D.create(privot.get_position(),  get_position())
